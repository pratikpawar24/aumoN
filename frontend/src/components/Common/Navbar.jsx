...
import {
  MapPin, Navigation, Users, BarChart3, User,
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/10">
          ...
          <div className="flex items-center gap-3">
            ...
            {isAuthenticated ? (
              <div className="relative">
                ...
                >
                  <div className="w-7 h-7 bg-primary-500 rounded-full flex items-center
                                  justify-content-center text-white text-xs font-bold
                                  flex items-center justify-center">
                    {user?.name?.[0]?.toUpperCase() || 'U'}
                  </div>
                  <span className="hidden sm:block text-sm text-primary-400 font-medium">
                    {user?.name?.split(' ')[0]}
                  </span>
                </button>

                {userMenuOpen && (
                  <div className="absolute right-0 mt-2 w-48 glass rounded-xl shadow-xl
                                  border border-white/10 py-1 animate-fade-in z-50">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-2 px-4 py-2 text-sm text-slate-300
              ...
              <div className="flex items-center gap-2">
                ...
                <Link to="/register"
                  className="px-4 py-2 bg-primary-500 text-white text-sm font-medium
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden glass border-t border-white/10 py-2 px-4 animate-slide-down">
          {navLinks.map(({ to, icon: Icon, label }) => (
            <Link
              key={to}
              to={to}
